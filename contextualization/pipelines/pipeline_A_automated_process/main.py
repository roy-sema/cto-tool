...
from contextualization.pipelines.pipeline_A_automated_process.models import (
    CommitCollection,
    DevelopmentActivityJustification,
...
class GitAnalysisResults(BaseModel):
    ...
    development_activity_by_repo: dict[str, list[DevelopmentActivityJustification]] | None = Field(default_factory=list)
    ...
    with calls_context("pipeline_a.yaml"):
        if run_for_commit_count:
            ...
            if total_code_commit_count > 0:
                logger.info("Non-zero code commits found.", extra={"commit_count": commit_count})
            else:
...
async def analyse_git_data(
    ...
    repo_group_summary_data_dfs = {}
    development_activity_by_repo = {}
    for repo_name in os.listdir(git_folder_path):
        ...
        if not diffs_with_summaries.is_empty():
            ...
            if not repo_group_git_repos:
                ...
                development_activity_by_repo["all_repos"] = await get_development_activity(diffs_with_summaries)
    ...
    if not repo_group_git_repos:  # Is None or empty
        return GitAnalysisResults(
            ...
            development_activity_by_repo=development_activity_by_repo,
        )

    for group_name, group_repos in repo_group_git_repos.items():
        logger.info("Processing repo group", extra={"repo_group_name": group_name, "repo_names": group_repos})
        diffs_with_summaries = all_repos_with_summaries.filter_by_repos(group_repos)
        if diffs_with_summaries.is_empty():
            logger.info("No commits for repo group", extra={"repo_group_name": group_name})
            continue
        ...
        development_activity_by_repo[group_name] = await get_development_activity(diffs_with_summaries)
...
def fill_already_analysed_diffs_collection(git_diffs: CommitCollection) -> CommitCollection:
    ...
    analysis_data = {
        record["sha"]: {
            ...
            "category_justification": record["category_justification"],
    ...
    for commit in git_diffs.commits:
        ...
        if data["category_justification"] is None:  # Backward compatibility
            continue

        commit.Summary = data["Summary"]
        commit.category = data["category"]
        commit.category_justification = data["category_justification"]
...
async def get_development_activity(git_summaries: CommitCollection) -> list[DevelopmentActivityJustification]:
    results = []
    for activity_type in DevelopmentActivityType:
        commits_for_category = [commit for commit in git_summaries.commits if activity_type.name == commit.category]
        if len(commits_for_category) == 0:
            continue

        category_justifications = "\n• " + "\n• ".join(
            commit.category_justification for commit in commits_for_category if commit.category_justification
        )
        results.append(
            DevelopmentActivityJustification(activity_type=activity_type, justification=category_justifications)
        )
