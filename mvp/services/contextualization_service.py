...
class ContextualizationService:
    ...
    def process_organization(
        ...
        try:
            ...
            with tracer.start_as_current_span(
                ...
                start_date, end_date = cls.get_start_and_end_date(day_interval, hour=cls.PIPELINES_START_HOUR)
                contextualization_results = ContextualizationResults(
                    analysis_period=cls.get_analysis_period(day_interval, hour=cls.PIPELINES_START_HOUR),
                )
                jira_params = None
                repo_group_git_repos, repo_group_jira_projects = None, None
                organization_groups = RepositoryGroup.objects.filter(organization=organization).exists()
                if not dry_run and not import_only:
                ...
                if cls.PIPELINE_A in pipelines and not import_only:
                    pipeline_a_result = cls.execute_pipeline_a(
                        ...
                        dry_run=dry_run,
                        force=force_pipeline_a,
                    )
                    contextualization_results.pipeline_a_result = pipeline_a_result
                    if pipeline_a_result is not None and not pipeline_a_result.total_code_commit_count:
                ...
                if cls.PIPELINE_BC in pipelines and not import_only and repo_group_git_repos is not None:
                    ...
                    pipeline_b_and_c_result = cls.execute_pipeline_b_and_c(
                        ...
                        organization=organization,
                        summary_data_dfs=(
                            contextualization_results.pipeline_a_result.summary_data_dfs
                            if contextualization_results.pipeline_a_result
                            else {}
                        ),
                        repo_group_git_repos=repo_group_git_repos,
                ...
                if cls.PIPELINE_ANOMALY_INSIGHTS in pipelines:
                    ...
                    if not import_only:
                        ...
                        combined_insights = cls.execute_pipeline_anomaly_insights(
                            ...
                            summary_data=(
                                contextualization_results.pipeline_a_result.summary_data_all_repos
                                if contextualization_results.pipeline_a_result
                                else None
                            ),
                            dry_run=dry_run,
    ...
    def get_start_and_end_date(
        ...
        end_date = datetime.combine(day, time()) if day else None

        if not end_date:
            end_date = datetime.fromisoformat(settings.MOCKED_END_DATE) if settings.MOCKED_END_DATE else datetime.now()

        # Replace time components in end_date
        end_date = end_date.replace(hour=hour, minute=minute, second=second, microsecond=microsecond)
    ...
    def get_analysis_period(
        ...
        start_date, end_date = cls.get_start_and_end_date(day_interval, hour, minute, second, microsecond, day)
        return AnalysisPeriod(start_date=start_date, end_date=end_date)
    ...
    def check_commits_exist_for_pipeline_a(
        ...
        """Check repository commits exist for pipelines A.
    ...
    def clean_previous_data(cls, organization: Organization, pipelines: set[str]):
        """Zip the output of the previous run and remove the directory removes the existing zip file if it exists."""
        logger.info(
    ...
    def execute_pipeline_anomaly_insights(
        cls, output_dir: Path, summary_data: CommitCollection | None = None, dry_run=False
    ) -> GitCombinedInsights | None:
        if not summary_data or summary_data.is_empty():
            raise ValueError("Pipeline A data not found. Did you run pipeline A?")
        if dry_run:
            logger.info(f"Would execute Pipeline Anomaly Insights")
            return None

        with execute_pipeline_with_telemetry("anomaly_insights"):
            return asyncio.run(run_anomaly_driven_insights(output_dir=output_dir, summary_data=summary_data))
    ...
    def execute_pipeline_insights_aggregation(
        ...
        if jira_anomaly_file and os.path.exists(jira_anomaly_file):
            files.append(jira_anomaly_file)
        else:
            logger.warning(
                "'jira_anomaly_file' not found. Did you run pipeline jira_anomaly_insights?",
                extra={"jira_anomaly_file": jira_anomaly_file},
            )
    ...
    @classmethod
    def execute_pipeline_d(cls, jira_params: dict, dry_run=False) -> PipelineDResult | None:
        if not jira_params:
        ...
        if dry_run:
            ...
            if "jira_access_token" in parameters_to_log:
                parameters_to_log["jira_access_token"] = "<redacted-token>"
            logger.info(f"Would execute Pipeline D: {parameters_to_log=}")
            return None
    ...
    def copy_output_files(
        ...
        for suffix, destination_filename in files.items():
            ...
            if not os.path.exists(script_output_path):
                logger.warning(
                    f"Script output file not found: {script_output_path}, script might had an error or there were no data to process."
                )
                continue
    ...
    def get_script_output_path(cls, data_dir: str, suffix: str):
        ...
        if suffix == cls.SCRIPT_OUTPUT_JIRA_ANOMALY_INSIGHTS_SUFFIX:
        ...
        if (
            suffix == cls.SCRIPT_OUTPUT_COMBINED_ANOMALY_INSIGHTS
            or suffix == cls.SCRIPT_OUTPUT_INSIGHTS_AGGREGATION
            or suffix == cls.SCRIPT_OUTPUT_JIRA_ANOMALY_INSIGHTS_SUFFIX
        ):
            filename = suffix
        else:
    ...
    def get_organizations(
        ...
        if skip_organization_ids:
            logger.warning(
                f"Skipping contextualisation for organizations {skip_organization_ids}",
    ...
    def get_jira_params(cls, organization: Organization, start_date: str, end_date: str) -> dict | None:
        """Return Jira parameters for the organization if Jira is connected.
        ...
        if not projects_keys:
            logger.info("No Jira project selected.", extra={"organization": organization.name})
            return None
    ...
    def generate_repository_group(cls, organization: Organization) -> tuple[dict[str, list[str]], dict[str, list[str]]]:
        ...
        for repo_group in repo_groups:
            ...
            # Git
            repositories = Repository.objects.filter(repository_group=repo_group).all()
