...
import BudgetPerformanceItem from "@/compass/components/BudgetPerformanceItem.vue";
import PageHeader from "@/compass/components/common/PageHeader.vue";
import { getBudgetPerformanceData } from "@/compass/helpers/api";
...
const metrics = ref<BudgetPerformanceMetric[]>();
...
const loadData = async () => {
  try {
    loading.value = true;
    const response = await getBudgetPerformanceData();
        ...
        <div class="flex flex-col gap-5">
          ...
          <template v-else>
            ...
            <template v-for="metric in metricsFiltered" :key="metric.name">
              <BudgetPerformanceItem :metric="metric" />
            </template>
          </template>
        </div>
      </Panel>
